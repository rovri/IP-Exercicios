...
bool preRequisitoFraco(ALUNOMATRICULADO* a1) {
	...
	if( a1->nota >= 30 && a1->frequencia >= 70) return true;
	else return false;
...
bool atualizarNotaEFrequencia(ALUNOMATRICULADO* a1, int nota, int freq) {
	...
	if(nota < 0 || nota > 100 || freq < 0 || freq > 100) return false;
...
void atualizarStatus(ALUNOMATRICULADO* a1) {
    ...
    if(a1->nota < 50 && a1->frequencia < 70) a1->status = 'E';

    else if(a1->nota < 30 && a1->frequencia >= 70) a1->status = 'N';

         else if(a1->nota >= 50 && a1->frequencia < 70) a1->status = 'F';

              else if(a1->nota >= 30 && a1->nota < 50 && a1->frequencia >= 70) a1->status = 'R';
...
void arredondarParaCinco(MINISTRANTE min1, int valorMinimo) {
	...
	for(int i = 0; i < min1.numAlunos; i++) {

        if(min1.alunos[i]->frequencia >= 70){

            if(min1.alunos[i]->nota < 50 && min1.alunos[i]->nota >= valorMinimo) min1.alunos[i]->nota = 50;
...
bool adicionarMinistrante(CURSO* c1, MINISTRANTE* min1){
    ...
    if(c1->numMinistrantes != 10){
        for(int i = -1; i < c1->numMinistrantes; i++){


            if(min1->pessoa.nusp == c1->ministrantes[i+1]->pessoa.nusp) return false;

            else {

                c1->ministrantes[c1->numMinistrantes] = min1;
